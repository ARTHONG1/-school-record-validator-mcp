...
function charShapeToStyleStandalone(charShape: ParsedCharShape, faceNames: Map<number, ParsedFaceName>): CharacterStyle {
  ...
  return {
    ...
    underline: charShape.underlineType !== 0 ? {
      type: ({ 0: 'None', 1: 'Bottom', 2: 'Center', 3: 'Top' } as Record<number, import('./types.js').UnderlineType>)[charShape.underlineType] || 'Bottom',
      shape: ({ 0: 'Solid', 1: 'Dash', 2: 'Dot', 3: 'DashDot', 4: 'DashDotDot', 5: 'LongDash', 6: 'CircleDot', 7: 'DoubleSlim', 8: 'SlimThick', 9: 'ThickSlim', 10: 'SlimThickSlim' } as Record<number, import('./types.js').LineType2>)[charShape.underlineShape] || 'Solid',
          ...
          if (cellRow < ctx.tableRowCount && cellCol < ctx.tableColCount && ctx.tableCells[cellRow]) {
            ...
            if (borderFill) {
              ...
              if (fill && fill.fillType === 'solid' && fill.backgroundColor !== undefined) {
                ...
                const gradTypeMap: Record<number, import('./types.js').GradationType> = { 1: 'Linear', 2: 'Radial', 3: 'Conical', 4: 'Square' };
