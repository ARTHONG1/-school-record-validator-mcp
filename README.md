...
2026학년도 **초등학교 학교생활기록부 문안**을 교육부 공식 문서에 근거해 검토하는 교육·실습용 MCP 서버입니다. AI가 7개 도구를 직접 호출해 금지·검토 표현, 입력 길이, 작성 근거, 관련 규칙과 원문 인용을 구조화된 결과로 확인합니다.
...
검증 결과는 문안 자체와 작성 경위 확인을 분리합니다. `contentStatus: pass`는 문안에서 현재 규칙팩의 금지·검토 표현이 탐지되지 않았다는 뜻이고, 작성 경위 정보가 빠진 경우에는 `needsContext`에 중립적인 확인 항목이 표시됩니다. 정보가 없다는 이유만으로 문안을 위반으로 판정하지 않습니다.

종합 `status`는 다음 우선순위를 따릅니다.

| 상태 | 의미 |
|---|---|
| `pass` | 문안과 작성 경위 확인이 모두 완료되었고 탐지된 항목이 없음 |
| `needs_context` | 문안 자체는 통과했지만 직접 관찰·사실성·작성 경위 확인 정보가 부족함 |
| `review` | 교사의 판단이 필요한 명시적 위험 또는 편집 검토 항목이 있음 |
| `blocked` | 공식 금지사항, 허위 사실, 허용되지 않는 작성 방식 등이 명시적으로 확인됨 |

`needs_context`는 기재 위반이나 위험 판정이 아니므로 경고·오류 색상 대신 “최종 기재 전 확인”으로 표시해야 합니다. 자세한 JSON 계약은 [검증 결과 계약](docs/validation-result-contract.md)을 참고하세요.
...
네. MCP를 지원하는 AI 클라이언트에 서버를 한 번 등록하면 AI가 도구 목록과 입력 스키마를 읽고 필요할 때 직접 호출합니다.

1. 사용자가 “이 행동특성 문안을 검토해줘”라고 요청합니다.
2. AI가 `validate_record_text`를 호출합니다.
3. 필요한 경우 `explain_record_rule`, `search_record_guidance`, `get_source_excerpt`를 이어서 호출합니다.
4. MCP가 판정, 규칙 ID, 문서 위치, 검증 인용을 반환합니다.
5. AI가 결과를 사용자에게 설명합니다.
...
| 도구 | 역할 |
|---|---|
| `validate_record_text` | 문안 1건의 판정, UTF-8 바이트 길이, 발견 항목과 근거 반환 |
| `validate_record_batch` | 최대 100건을 검사하고 입력 `entryId`를 결과에 보존 |
| `search_record_guidance` | 사람이 승인한 초등·공통 공식 원문 청크 검색. 필요하면 `sourceRoles`로 교육부 기재요령·훈령 본문 등을 좁힘 |
| `get_source_excerpt` | 청크 원문, 문서 해시, 조항·별표·인쇄/PDF 쪽수 반환 |
| `explain_record_rule` | 규칙 범위, 탐지 방식, 예외, 권고와 모든 근거 반환 |
| `list_record_fields` | 지원 항목과 고정 입력 한도 또는 `system-range` 반환 |
| `rule_pack_info` | 학교급, 학년도, 원본 목록과 봉인 데이터 해시 반환 |
...
```text
school-record-validator MCP의 validate_record_text 도구를 반드시 사용해서
“교외 영어경시대회에서 대상을 수상했으며 전교에서 가장 성실함”을
2026학년도 초등학교 행동특성 및 종합의견 기준으로 검사하고,
발견된 규칙의 공식 근거도 설명해 줘.
```
